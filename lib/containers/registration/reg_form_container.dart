...
import 'package:flutter_starter_v2/actions/reg_actions.dart';
  ...
  static _ViewModel fromStore(Store<AppState> store) {
    return new _ViewModel(
        unameCheck: (String unamecheck) async {
          ...
          } else if (check.docs.isEmpty ||
              check.docs.single.data()['uname'] ==
        ...
        usernameController: new TextEditingController.fromValue(
            new TextEditingValue(
                text: store.state.currentUser.data()['uname'] ?? "",
                selection: new TextSelection.collapsed(
                    offset:
                        store.state.currentUser.data()['uname']?.length ?? 0))),
        ...
        stateController: new TextEditingController.fromValue(new TextEditingValue(
            text: store.state.currentUser.data()['stt'] ?? "",
            selection: new TextSelection.collapsed(
                offset: store.state.currentUser.data()['stt']?.length ?? 0))),
        countryController: new TextEditingController.fromValue(
            new TextEditingValue(text: store.state.currentUser.data()['country'] ?? "", selection: new TextSelection.collapsed(offset: store.state.currentUser.data()['country']?.length ?? 0))),
